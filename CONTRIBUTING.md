...
2. All commits must have `Signed-off-by` (e.g. `Signed-off-by: Joan Doe <joan.doe@email.com>`) in the commit message (details in Signing section).
...
We've chosen to use the Developer's Certificate of Origin (DCO) method that is employed by the Linux Kernel Project, which provides a simple way to contribute to Ansible-Lockdown projects.
